...
aws sqs create-queue \
  ...
  --region $REGION > /dev/null 2>&1 || echo "  DLQ already exists"

sleep 2  # wait for SQS propagation
...
# 2. Create Main SQS Queue (RedrivePolicy via JSON file to avoid escaping issues)
echo "[2/8] Creating SQS Main Queue..."

# Write RedrivePolicy to a temp file — avoids inline JSON escaping bugs
cat > /tmp/sqs-attrs.json << SQSEOF
{
...
aws sqs create-queue \
  ...
  --region $REGION > /dev/null 2>&1 || echo "  Queue already exists"
...
aws s3 mb s3://${BUCKET_NAME} --region $REGION > /dev/null 2>&1 || echo "  Bucket already exists"
...
aws athena create-work-group \
  ...
  --region $REGION > /dev/null 2>&1 || echo "  Workgroup already exists"
...
aws iam put-role-policy \
  ...
  --policy-document file://$(dirname $0)/iam_policy.json > /dev/null 2>&1 || true
...
SNS_ARN=$(aws sns create-topic \
  ...
  aws sns list-topics \
    --query "Topics[?contains(TopicArn,'feature-store-alerts')].TopicArn" \
    --output text)
